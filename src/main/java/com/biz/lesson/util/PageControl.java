...
public class PageControl {
	...
	protected void init(HttpServletRequest request) {
		...
		if (appendParams != null) {
			...
			appendParams = "";
		request.setAttribute(Constants.PAGE_CONTROL, this);
	}
	...
	public void setCount(long c) {
		if (c <= 0)
			return;
		this.count = Long.valueOf(c).intValue();
	...
	public Pageable toPageRequest() {
		return new PageRequest(this.getCurrentPage()-1, getPageSize());
	}

	public PageRequest toPageRequest(Sort sort){
		return new PageRequest(getCurrentPage() - 1, getPageSize(),sort);
	}
